...
class Analysis():
  ...
  def _makeHistogram(self):
    ...
    histoAxis.hist(self.activationEnergies, bins="auto", color='grey', alpha=0.7, edgecolor='black', linewidth=1.0, label='Activation Energy')
    histoAxis.axvline(x=self.averageActivationEnergy, color='blue', linestyle='--', label='Average Activation Energy')
    histoAxis.text(self.averageActivationEnergy+0.05, -0.25, f'{self.averageActivationEnergy:.4f}', va='bottom', ha='right', color='blue')
    histoAxis.set_xlabel('Activation Energy (eV)', fontsize=14)
    histoAxis.set_ylabel('Count', fontsize=14)
    histoAxis.set_title(f'Activation Energy Histogram at {self.temperature} K', fontsize=20)
